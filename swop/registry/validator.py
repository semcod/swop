...
def validate_contract(
    contract: Contract, *, root: Optional[Path] = None
) -> ValidationResult:
    ...
    *root* is the project root used to resolve relative paths in ``layers``.
    ...
    if "command" in raw:
        ...
        if kind not in ("DOMAIN_EVENT", "INTEGRATION_EVENT"):
            errors.append(
                f"event kind must be DOMAIN_EVENT or INTEGRATION_EVENT, got {kind!r}"
            )
    else:
        errors.append(
            "missing discriminator: one of 'command', 'query', 'event' required"
        )
...
def _check_layer_paths(
    raw: Dict[str, Any], root: Optional[Path], errors: List[str]
) -> None:
